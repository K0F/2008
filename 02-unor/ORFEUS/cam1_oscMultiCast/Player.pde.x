...
class Player{
	...
	String address =  "192.168.0.2";//"78.102.53.186";
	int port = 5000;
	...
	void draw(){
		...
		*/
		...
		ruch[2] += (ruch[1]-ruch[2]) / 5.0;
		ruch[2] = round(ruch[2]*1000.0)/1000.0;
		float result = map(ruch[2],0.0,0.2,0.0,1.0);	
		...
		if(cp.mute){
			send("/rush",result);
			send("/x",map(avgX,0,width,0,1.0));
			send("/y",map(avgY,0,height,1.0,0));
		}
