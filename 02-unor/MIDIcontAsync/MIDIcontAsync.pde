...
class Knob{
	...
	color Cback = #22FFFFFF;
	color Cstroke = 0;
	color Cactive = #FFFFCC22;
	color Cmacroized = #CCFFCC22;
	color Crecording = #FFFF1122;
	...
	boolean rec(){
		...
		if(over(10)&&(mousePressed)&&(mouseButton==RIGHT)){
			answr = true;
			someData = true;
			recordTime=vals.length+1;
			vals = (int[])expand(vals,recordTime);
			target = (int)map(mouseY,y,y-h,0,127);
			target = constrain(target,0,127);
			vals[recordTime-1] = target;
...
class Keyboard{
	...
	Keyboard(){
		for(int i = 0;i<notes.length;i++){
			if(i<9){
				notes[i] = new Note((int)map(i%9,0,keys.length/3.0f,20,110),100,50);
			}else if(i<18){
				notes[i] = new Note((int)map(i%9,0,keys.length/3.0f,20,110),100,100);
			}else{
				notes[i] = new Note((int)map(i%9,0,keys.length/3.0f,20,110),100,400);
			}
		}
	}
