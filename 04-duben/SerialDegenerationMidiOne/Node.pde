class Node{
  ...
  void draw(){
    ...
    triangle(x,y,x+3*w,y,(node[which].x+(1.5f*w)),(node[which].y+(1.5f*w)));
  ...
  float[][] mutate(float[][] _matix){
    ...
    tone/=sq(mat);
    play((int)tx/2/*(int)tone/2*/,(int)ty,(int)(dist(node[which].tx,
    node[which].ty,tx,ty)*3));
  ...
  Note note;
    note = new Note(constrain(a,0,127),constrain(b,0,127),constrain(c,0,127));
    //println(100-(x+10)*2);
