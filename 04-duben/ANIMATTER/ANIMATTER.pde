...
void setup(){
  ...
  in = new Capture(this, W, H,"Labtec WebCam-WDM");
...
void export(){
 mm = new MovieMaker(this, width, height, "out"+filename+".mov",30, MovieMaker.JPEG, MovieMaker.HIGH);
...
void capture(){
  ...
  for(int x=0;x<W;x++){
    for(int y=0;y<H;y++){
      tmp.pixels[y*W+x] = color(brightness(in.pixels[y*W+x]));
    }
  }
  if(cyklon.length<1000){
    ...
    //tmp.save("out/captOne"+filename+".png");
    //
    //filename++;
  }else{
    export();
