class SnuraPhy{
  ...
  SnuraPhy(int _id,int _pocet,Point A,Point B,float _mass, float _sila,float _pruznost,float _vule,boolean _klen){
    id = _id;
    pts = new Point[2];
    pts[0] = A;
    pts[1] = B;
  ...
  void reset(){    
   ...
   if(!fixed){
    ...
    particles[particles.length-1] = parents[1];//physics.makeParticle( 1.0, pts[1].x,pts[1].y,pts[1].z);
     ...
     particles[0] = physics.makeParticle( mass, pts[0].x,pts[0].y,pts[0].z);
    particles[particles.length-1] = physics.makeParticle( mass, pts[1].x,pts[1].y,pts[1].z);
   }
  ...
  boolean over(){
    ...
    for(int i  = 0;i<flatX.length;i++){
      if(dist(flatX[i],flatY[i],mouseX,mouseY)<15){
      answr = true;
      }
    }    
    return answr;
